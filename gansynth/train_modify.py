...
class PGGAN(object):
        ...
        for cur_level in range(7):
            ...
            if cur_level == 0:
                ...
                for step in range(self.stable_steps * 2):
                    ...
                    if cur_level == 6:
                        torch.save(self.g_net.state_dict(), folder + '/Gnet_%dx%d_step%d.pth' % (
                            2 ** (cur_level + 1), 2 ** (cur_level + 4), step))
                        torch.save(self.d_net.state_dict(), folder + '/Dnet_%dx%d_step%d.pth' % (
                            2 ** (cur_level + 1), 2 ** (cur_level + 4), step))
            torch.save(self.g_net.state_dict(),
                       folder + '/Gnet_%dx%d.pth' % (2 ** (cur_level + 1), 2 ** (cur_level + 4)))
            torch.save(self.d_net.state_dict(),
                       folder + '/Dnet_%dx%d.pth' % (2 ** (cur_level + 1), 2 ** (cur_level + 4)))
    ...
    def _train(self, net_level, net_status, net_alpha, cur_step):
        ...
        for batch_idx in range(int(TOTAL_DATA_SIZE / BATCH_SIZE)):
            ...
            for batch_idx2, (pitch_label, mel_spec, mel_IF) in enumerate(train_loader):
                ...
                # Train D with GP
                gradient_penalty = 10 * self._gradient_penalty(stack_real_image.data, fake_generated_sample.data,
                                                               little_batch_size, current_level_res)
                ...
                self.g_optim.step()
                if batch_idx % 1400 == 0:
                    self.generate_picture(fake_generated_sample[:, 0, :, :], current_level_res, cur_step, batch_idx,
                                          net_status)
                if batch_idx % 200 == 0:
                    print("Resolution:{}x{}, Status:{}, Cur_step:{}, Batch_id:{},"
                          "D_loss:{}, W_D:{}, M_fake:{},M_Real:{}, GP:{}, Real_P_Loss:{},"
                          "Fake_P_Loss:{}, G_loss:{}, Net_alpha:{}".format(current_level_res,
                                                                           current_level_res * (2 ** 3),
                                                                           net_status, cur_step, batch_idx, D_loss,
                                                                           Wasserstein_D, mean_fake, mean_real,
                                                                           real_pitch_loss,
                                                                           fake_pitch_loss, gradient_penalty, G_loss,
                                                                           net_alpha))
    ...
    def generate_picture(self, spec, resolution, step, batch_idx, status):
        ...
        plt.savefig(
            folder + "/{}_{}_{}_{}_{}_sample.png".format(resolution, resolution * (2 ** 3), status, step, batch_idx))
