...
TOTAL_DATA_SIZE = 10368  # new dataset
...
class PGGAN(object):
    def __init__(self,
        ...
        self.gradient_weight_real_ = torch.FloatTensor([-1]).to(device)
        self.gradient_weight_fake_ = torch.FloatTensor([1]).to(device)
        self._init_network()
...
#                     self._train(cur_level, net_status, net_alpha, step)
        ...
        while batch_idx < int(TOTAL_DATA_SIZE / BATCH_SIZE):
            if batch_idx == (8000 / BATCH_SIZE):
                train_loader = udata.DataLoader(udata.TensorDataset(
                                    torch.Tensor(np.load('../data/np_array_train_MS/' + 'pitch_list_' + str(TOTAL_DATA_SIZE) + '.npy')),
                                    torch.Tensor(np.load('../data/np_array_train_MS/' + 'mel_spec_list_' + str(TOTAL_DATA_SIZE) + '.npy')),
                                    torch.Tensor(np.load('../data/np_array_train_MS/' + 'mel_IF_list_' + str(TOTAL_DATA_SIZE) + '.npy'))
                ...
                train_loader = udata.DataLoader(udata.TensorDataset(
                                    torch.Tensor(np.load('../data/np_array_train_MS/' + 'pitch_list_' + str(batch_idx * BATCH_SIZE + 4000) + '.npy')),
                                    torch.Tensor(np.load('../data/np_array_train_MS/' + 'mel_spec_list_' + str(batch_idx * BATCH_SIZE + 4000) + '.npy')),
                                    torch.Tensor(np.load('../data/np_array_train_MS/' + 'mel_IF_list_' + str(batch_idx * BATCH_SIZE + 4000) + '.npy'))
                ), batch_size=BATCH_SIZE, shuffle=True)
            ...
            for batch_idx2, (pitch_label, mel_spec, mel_IF) in enumerate(train_loader):
                ...
                stack_real_image = torch.stack((spec_L, IF_L, spec_R, IF_R), dim=1).to(device)
                stack_real_image = torch.transpose(stack_real_image, 2, 3)
                little_batch_size = spec_L.size()[0]
                stack_real_image = stack_real_image.to(device)
                ...
                fake_seed = torch.randn(little_batch_size, self.latent_size_, 1, 1).to(device)
    ...
    def _init_network(self):
        ...
        self.g_net = Generator(256, self.latent_size_, self.rgb_channel_,
                                is_tanh=self.is_tanh_, channel_list=[256, 256, 256, 256, 256, 128, 64, 32]).to(device)
        self.d_net = Discriminator(256, self.rgb_channel_,
                                   is_sigmoid=self.is_sigmoid_, channel_list=[256, 256, 256, 256, 128, 64, 32, 32]).to(device)
        # if TO_GPU:
        # self.g_net.cuda(0)
        # self.d_net.cuda(0)
        print(self.g_net)
    ...
    def _gradient_penalty(self, real_data, fake_data, batch_size, res):
        ...
        gradients = torch.autograd.grad(outputs=d_median_data, inputs=median_data,
                                        grad_outputs=torch.ones(d_median_data.size()).to(device),
