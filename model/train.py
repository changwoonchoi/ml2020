        ...
        while batch_idx < int(TOTAL_DATA_SIZE/BATCH_SIZE):
            if batch_idx == (24000/BATCH_SIZE):
                train_loader = udata.DataLoader(udata.TensorDataset(
                                    torch.Tensor(np.load('../np_array_MS/'+'pitch_list_'+ str(TOTAL_DATA_SIZE) + '.npy')),
                                    torch.Tensor(np.load('../np_array_MS/'+'mel_spec_list_' + str(TOTAL_DATA_SIZE) + '.npy')),
                                    torch.Tensor(np.load('../np_array_MS/'+'mel_IF_list_' + str(TOTAL_DATA_SIZE) + '.npy'))
                ...
                train_loader = udata.DataLoader(udata.TensorDataset(
                                    torch.Tensor(np.load('../np_array_MS/'+'pitch_list_'+ str(batch_idx*BATCH_SIZE+4000) + '.npy')),
                                    torch.Tensor(np.load('../np_array_MS/'+'mel_spec_list_' + str(batch_idx*BATCH_SIZE+4000) + '.npy')),
                                    torch.Tensor(np.load('../np_array_MS/'+'mel_IF_list_' + str(batch_idx*BATCH_SIZE+4000) + '.npy'))
                                           ), batch_size=BATCH_SIZE, shuffle=True)
